  ...
  try {
    ...
    (async () => {
      try {
        ...
        // Generate scaffold via AIPipe
        let generatedFiles = null;
        try {
          const prompt = `Generate a minimal static single-file app for the following brief:\n${brief || ""}\nReturn only source code as a JSON map of filename->content.`;
          const aiResp = await axios.post(
            process.env.AIPIPE_ENDPOINT,
            { prompt, max_tokens: 2000 },
            { headers: { Authorization: `Bearer ${process.env.AIPIPE_KEY}` } }
          );
          generatedFiles = aiResp.data && aiResp.data.files;
        } catch (e) {
          console.warn("AIPipe call failed:", e.message || e);
        }
